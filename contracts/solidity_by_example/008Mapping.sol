...
contract Mapping{
    ...
    // 声明 映射 的 格式为 mapping(_KeyType => _ValueType)，其中_KeyType 和 _ValueType 分别是 Key 和 Value 的 变量类型。
    ...
    // 规则1：映射 的 映射键_KeyType 只能选择 Solidity 内置的 值类型，比如uint，address等，不能用自定义的结构体。
    // 下面这个例子会报错，因为 映射键_KeyType 使用了 我们 自定义的结构体：
    // mapping 中 只允许基本类型、用户定义的值类型、契约类型 或 枚举 作为 映射键。
    struct Student{
        uint256 id;
        uint256 score;
    }
...
// 通过 一个人 的 id 来查询 他的钱包地址，这在处理用户信息、权限管理、资源分配 等场景中尤为重要。
